...
```mermaid
flowchart LR
  main([apache main db7adf0a])

  merged["MERGED upstream 2026-07: OPENNLP-1868 case folding · OPENNLP-1869 emoji normalization<br/>OPENNLP-1876 de-regex normalizers · OPENNLP-1890 allowlist model loading"]
  main --- merged

  %% ---- open pull requests against apache/opennlp ----
  main --> p1150["#1150 · OPENNLP-1875 · UCD whitespace"]
  main --> p1161["#1161 · OPENNLP-1878 · hot-path performance ✓ approved"]
  ...
  class p1150,p1177,d1152,d1154,d1155,d1165,d1166,d1167 draft;
...
| [#1150](https://github.com/apache/opennlp/pull/1150) | OPENNLP-1875 | `StringUtil` whitespace on the Unicode White_Space property | Open, re-review pending | |
| [#1161](https://github.com/apache/opennlp/pull/1161) | OPENNLP-1878 | Hot-path performance rewrites | Approved | Roughly 2x on the touched paths, measured per-path on the PR |
...
|---|---|---|---|
| `depparse` | Transition-based dependency parsing: classical perceptron tiers plus a feedforward neural tier with beam decoding | Staged | UD English EWT test, gold UPOS: 86.78 UAS / 84.61 LAS at beam 4; all-neural pipeline 83.73 / 79.88 at 448 tok/s. The published Stanza end-to-end reference on the same treebank is 88.90 / 86.77, so this is not yet at parity; the tagger is the dominant gap |
| `ff-postagger` | Feedforward neural POS tagger on the same trainer recipe | Staged | 94.68% on UD English EWT vs 93.75% for the best classical configuration in-tree |
