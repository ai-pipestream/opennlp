...
public class FeatureGeneratorUtil {
   ...
   * <li>2d - contains two digits </li>
   * <li>4d - contains four digits </li>
   ...
   * <li>num - digits </li>
   * <li>sc - single uppercase Unicode code point </li>
  ...
  public static String tokenFeature(String token) {
    ...
    else if (isCapitalWithPeriod(token)) {
      feat = "cp";
    }
  ...
  private static boolean isCapitalWithPeriod(String token) {
    if (token.isEmpty()) {
      return false;
    }

    int first = token.codePointAt(0);
    int periodOffset = Character.charCount(first);
