...
public class SimpleEventStreamBuilder {
  ...
  private static final String FORMAT_ERROR = "format error of the event \"";
  ...
  public SimpleEventStreamBuilder add(String event) {
    ...
    if (slash < 1) {
      throw new IllegalArgumentException(FORMAT_ERROR + event + '"');
    }
    String outcome = event.substring(0, slash);

    String[] cvPairs = EventFields.split(event.substring(slash + 1));
    if (cvPairs.length == 0) {
      throw new IllegalArgumentException(FORMAT_ERROR + event + '"');
    }
