 ...
 * the test tree so Lucene stays a bench-scope dependency, never a runtime one.
 *
...
public final class HnswFloatIndex implements VectorIndex, AutoCloseable {
  ...
  public void freeze() {
    ...
    try {
      ...
      // Plain per-extension files, so bytesPerVector() can read the vector and graph sizes.
  ...
  /**
   * {@return the in-memory bytes per indexed vector: the vector data, vector metadata, and
